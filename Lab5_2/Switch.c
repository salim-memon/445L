...
void Switch_Init(void){ 
  ...
  GPIO_PORTE_DIR_R |= 0x30;        // maake PortE pins 0-3 input switches and pe4 output (sound)
  GPIO_PORTE_AFSEL_R &= ~0x1F;     // disable alt funct on PE4-0
...
void Switch_Poll(void){
	...
	if(Board_Input() == 0x01){			 // see if switch1 pressed
	 while(Board_Input() == 0x01){} //waiting for switch to be released
			if(play_song == false){
				Play(); 
				}
			if(play_song == true){
			...
			else if(Board_Input() == 0x10){	 // see if switch2 pressed
				Pause();  
				while(Board_Input() == 0x10){} //waiting for switch to be released
					Reset(); //reset song pointer
						}
			
						else if(Board_Input() == 0x00){	 //see if both switches pressed
							  while(Board_Input() == 0x00){} //waiting for switch to be released
								Pause();
								Mode_Instrument(); // choose instrument mode
								Play(); 
								}
