...
void Switch_Init(void){ 
  ...
  GPIO_PORTA_AMSEL_R &= ~0x0F;      // 3) disable analog on PE?
  GPIO_PORTA_PCTL_R &= ~0x000F0F00; // 4) PCTL GPIO on PA5
  GPIO_PORTA_DIR_R &= ~0x0F;        // 5) direction PA5 input
  GPIO_PORTA_AFSEL_R &= ~0x0F;      // 6) PA5 regular port function
  GPIO_PORTA_DEN_R |= 0x0F;         // 7) enable PA5 digital port
}
//------------Switch_Input------------
